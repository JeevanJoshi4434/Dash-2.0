...
class User {
    ...
    // Create a new user
    public async createUser(name: string, email: string, password: string, type: UserType, location: Location): Promise<IUser> {
        const user = new this.userModel({ name, email, password, type, location, lastActive: new Date() });
        return user.save();
    }
    ...
    // Find user by a custom field (e.g., type or email)
    public async findUserByField(field: string, value: string): Promise<IUser | null> {
        const validFields = ['_id', 'email', 'type']; // Only allow specific fields to be queried
